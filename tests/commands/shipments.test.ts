...
import { createShipmentsCommand } from "../../src/commands/shipments.js";
...
describe("shipments command", () => {
  const cmd = createShipmentsCommand();

  it("registers as 'shipments' with a description", () => {
    expect(cmd.name()).toBe("shipments");
    expect(cmd.description()).toMatch(/shipment|expedition/i);
  });
  ...
  it("validate accepts --no-trigger", () => {
    ...
    // --no-trigger registers as the 'trigger' option internally
    const f = flags(validate);
    expect(f).toContain("--no-trigger");
  });

  it("close takes a shipment id", () => {
